...
| 3.3 | `3 frontend · 3.3 camera capture` | #11b camera → C2PA + OTS, **owner upgraded scope: full product, integrated + verified** — capture UI (`web/src/app/capture/`), upload through the EXISTING `/media` + C2PA/OTS pipeline (no parallel pipeline, no new workers), captured media lands as a verified block on the entity profile + `/e/[slug]`, proof visible via `/proof` + MCP `teta_get_proof`. Was ⏸ after 5.2 — de-coupled 2026-07-12: the split is frozen indefinitely and camera only needs the existing monorepo pipeline. Signing/stamping is per-request one-off load (same as existing block flow), OK on current droplet | 🟢 ready (unfrozen) | `ttpi-wt/3.3-camera` · new files `web/src/app/capture/` + minimal wiring in profile/e pages |
