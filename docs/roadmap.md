...
| 1.7 | `1 backend · 1.7 verification hygiene bundle` | audit #6 (PATCH keeps `agent_endpoint_verified` after endpoint change → reset), #7 (unauthenticated SSRF-prone `/verify-endpoint` → auth/rate-limit the fetch — **15.1 CONFIRMED live on prod 2026-07-13**, unauth GET of `https://example.com` returned `is_active:true`), #13 (Redis `GETDEL` for code confirm). Same family as 1.5 — can be one session with it | 🔴 **live confirmed SSRF — bump priority with 1.6** | `routes/businesses.py`, `routes/endpoint_verification.py`, `services/verification/*` |
