...
| 1.12 | `1 backend · 1.12 fix /businesses/{id}/preview 500` | 🔴 found in 2.5 live E2E: `GET /businesses/{id}/preview` (agent_preview) 500s → breaks 3/7 MCP tools (`teta_get_profile`, `teta_verify_claim`, `teta_verify_endpoint`). Backend-side (handler/schema), confirmed via curl. Fix + regression-check all 7 tools | 🔴 high (blocks MCP surface) | `routes/businesses.py` (preview handler) |
