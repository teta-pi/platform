...
**1 backend · 2 mcp · 3 frontend (product UI) · 4 db · 5 devops · 6 manager · 7 github · 8 analytics · 9 server · 10 landing (promo) · 11 backoffice · 12 wordpress (plugin) · 13 gtm (autonomous go-to-market) · 14 camera (PI Camera product)**. Historical ✅ work
...
| 1.4 | `1 backend · 1.4 TWIRA source_weight` | per-method trust weights (registry/email/domain/document) feeding T-component; account for the weak email-domain binding noted in known-issues (1.3 review) | 🟢 ready (1.3 merged) · small | `api/app/twira/*` |
...
| 3.4 | `3 frontend · 3.4 verification methods UI` | rework UI (`docs/verification-rework.md`): method chooser (registry/email/domain active; document visible-DISABLED "Coming soon"); brand↔legal link UI; public disclosure of the link on profile + `/e/[slug]` | 🟢 ready (1.3 merged) | claim/profile verification UI |
...
| 14.1 | `14 camera · 14.1 PI Camera full product` | (ex-3.3, scope upgraded) capture UI (`web/src/app/capture/`) → EXISTING `/media` + C2PA/OTS pipeline (no parallel pipeline, no new workers) → verified block on profile + `/e/[slug]`, proof via `/proof` + MCP `teta_get_proof`; three use-cases: proof-of-creation, proof-of-process, copyright deposit. GTM tie-in: every signed asset's C2PA manifest links the creator's TETA+PI profile (Phase-3 C2PA loop). Per-request load only, OK on droplet | 🟢 ready | `ttpi-wt/14.1-camera` · new `web/src/app/capture/*` + minimal wiring profile/e + `lib/api.ts` (append) |
