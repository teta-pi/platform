...
| 1.15 | `1 backend · 1.15 fix resolve_intent proof_url` | found by 2.6 (2026-07-14): `teta_resolve_intent`'s `proof_url` (`api/app/api/routes/intent.py:76`) builds `/businesses/{slug}/proof` but the route is UUID-typed — dead link (422) since 2.1 shipped. Swap to `biz.id`, same fix pattern as 1.13's #2 | 🟡 small, isolated | `routes/intent.py` |
